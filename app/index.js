...
d3.select("#deathsTable").selectAll(".card")
    ...
    .html(function(d) {
        ...
        if (d.Mental == "Yes") {
            mentalStatus = "<span class='hideThis'>" + d.Mental + "</span><div class='mentalSwatch'>Mental health issues</div>";
        }
        if (d.URL != "#") {
...
function spitDots(year) {
    ...
    d3.select("#y" + year).selectAll(".dot")
        .data(dataShootings.filter(function(d) {
            return d.year == year;
        }).sort(function(a, b) {
        ...
        .call(d3.helper.tooltip(function(d, i) {
            var mentalStatus = "";
            if (d.Mental == "Yes") {
                mentalStatus = "<div class='mentallabel' style='color:#999;font-weight:900;font-size:.8em;'>MENTAL HEALTH ISSUES</div>";
            }
    ...
    function countStats(target, text) {
        if (mentalLoad == true) {
            return $(target).filter(function() {
                return $(this).text().toUpperCase().indexOf(text.toUpperCase()) != -1 && $(this).parent().find(".mentalSwatch").length
            }).length;
        } else {
            return $(target).filter(function() {
                return $(this).text().toUpperCase().indexOf(text.toUpperCase()) != -1
            }).length;
        }
    }
    $(".filter").on('click', function() {
        ...
        $('.dot').each(function(i, obj) {
            if (mentalLoad == true) {
                if ($(this).attr("mental") == "Yes") {
                    if (datapoint == "WHITE") {
                        if ($(this).attr("race") == "HISPANIC" || $(this).attr("race") == "WHITE") {
                            $(this).addClass("mentalSwatch");
                        }
