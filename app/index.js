...
$(document).ready(function() {
  ...
  var dataShootings = dataLoadShootings.mn_shootings;
  var dataChatter = dataLoadChatter.viz_chatter;
  ...
  d3.select('#deathsTable')
    ...
    .html(function(d) {
      ...
      if (d.Mental == 'Yes') {
        mentalStatus = `
          <span class="hideThis">${d.Mental}</span>
          <div class="mentalSwatch">Mental health issues</div>
        `;
      }
      if (d.URL != '#') {
      ...
      <div class="leftSide">
        <div class="name">
          ${d.FirstName || ''} ${d.LastName || ''} ${d.Suffix || ''}
        </div>

        <div class="stats">
          ${d.AgeYears || ''} &#8226; ${d.Occupation || ''}
        </div>

        <div class="description_chatter">
          ${d.StribNarrative || ''}
        </div>

        <div class="stats">
          ${d.Race || ''} &#8226; ${d.Gender || ''} &#8226; ${d.ResCity || ''}
        </div>

        <div class="location">
          Incident: ${d3.timeFormat('%m-%d-%Y')(new Date(d.DeathDate))} &#8226;
          Location: ${d.InjuryCity || ''} &#8226;
          Agency: ${d.Agency || ''}
        </div>

        <div class="ruling">
          Weapon: ${d.Weapon || ''} &#8226
          Ruling: ${d.MannerDeath || ''}
        </div>
      ...
      <div class="rightSide">
        <div class="photo">
          <img src="http://stmedia.stimg.co/${
  d.photo
}?h=155&w=150&fit=crop&bg=999&crop=faces" />
        </div>
      </div>
    ...
    d3.select('#y' + year)
      ...
      .data(
        dataShootings
          ...
          .sort(function(a, b) {
            return d3.ascending(a.Mental, b.Mental);
          })
      )
      .enter()
      .append('div')
      ...
      .attr('manner', function(d) {
        return d.MannerDeath;
      })
      .attr('region', function(d) {
        return d.Region;
      })
      .attr('weapon', function(d) {
        return d.WeaponCategory;
      })
      .attr('mental', function(d) {
        return d.Mental;
      })
    ...
    return function(selection) {
      ...
      selection
        .on('mouseover', function(d, i) {
          ...
          tooltipDiv
            .style('left', absoluteMousePos[0] - 180 + 'px')
  ...
  $('#filter_box').keyup(function(i) {
    if (mentalLoad == true) {
      $('#filter_box').attr(
        'placeholder',
        'Search list of mental health deaths by name, year or keyword'
      );
      $('.card').hide();
      var findIt = 'Yes';
      ...
      $('#filter_box').attr(
        'placeholder',
        'Search list of all deaths by name, year or keyword'
      );
      $('.card').hide();
      var txt = $('#filter_box').val();
      $('.card').each(function() {
  ...
  //FILTER STUFF
  $(
    '#mainFilters .filter3 a, #mainFilters .filter2 a, #mainFilters .filter a'
  ).css('background', '#fff');
  // $(document).bind('DOMNodeInserted', function(event) {
    ...
    $('.dot').each(function(i, obj) {
      if (mentalLoad == true) {
        if ($(this).attr('mental') == 'Yes') {
          if (datapoint == 'WHITE') {
            if (
              $(this).attr('race') == 'HISPANIC' ||
              $(this).attr('race') == 'WHITE'
            ) {
              $(this).addClass('mentalSwatch');
            }
          }
          else if (
            $(this).attr('gender') == datapoint ||
            $(this).attr('race') == datapoint ||
            $(this).attr('manner') == datapoint
          ) {
            $(this).addClass('mentalSwatch');
          }
        }
  ...
  if ($.urlParam('mental') != 0) {
    ...
    if ((page = 'yes')) {
      ...
      for (var i = 0; i < dataChatter.length; i++) {
        if (dataChatter[i].category == 'mental') {
          $('.chatterThing').html(dataChatter[i].chatter);
        }
      }
      $('.bigswitch, .switch').removeClass('selected');
      $('#mentalB_m, #mentalB').addClass('selected');
  ...
  $('#weaponB').on('click', function() {
    ...
    $('.dot').each(function(i, obj) {
      if (mentalLoad == true) {
        if (
          $(this).attr('weapon') == 'GUN' &&
          $(this).attr('mental') == 'Yes'
        ) {
          $(this).addClass('mentalSwatch');
          for (var i = 0; i < dataChatter.length; i++) {
            if (dataChatter[i].category == 'weapon_mental') {
              $('.chatterThing').html(dataChatter[i].chatter);
            }
          }
        }
        ...
        if ($(this).attr('weapon') == 'GUN') {
          $(this).addClass('mentalSwatch');
        }
        for (var i = 0; i < dataChatter.length; i++) {
          if (dataChatter[i].category == 'weapon_all') {
            $('.chatterThing').html(dataChatter[i].chatter);
          }
        }
      }
  ...
  $('#raceB').on('click', function() {
    ...
    $('.dot').each(function(i, obj) {
      if (mentalLoad == true) {
        if (
          $(this).attr('race') == 'BLACK' &&
          $(this).attr('mental') == 'Yes'
        ) {
          $(this).addClass('mentalSwatch');
          for (var i = 0; i < dataChatter.length; i++) {
            if (dataChatter[i].category == 'race_mental') {
              $('.chatterThing').html(dataChatter[i].chatter);
            }
          }
        }
        ...
        if ($(this).attr('race') == 'BLACK') {
          $(this).addClass('mentalSwatch');
        }
        for (var i = 0; i < dataChatter.length; i++) {
          if (dataChatter[i].category == 'race_all') {
            $('.chatterThing').html(dataChatter[i].chatter);
          }
        }
      }
  ...
  $('#placeB').on('click', function() {
    ...
    $('.dot').each(function(i, obj) {
      if (mentalLoad == true) {
        if (
          $(this).attr('region') == 'Metro' &&
          $(this).attr('mental') == 'Yes'
        ) {
          $(this).addClass('mentalSwatch');
          for (var i = 0; i < dataChatter.length; i++) {
            if (dataChatter[i].category == 'location_mental') {
              $('.chatterThing').html(dataChatter[i].chatter);
            }
          }
        }
        ...
        if ($(this).attr('region') == 'Metro') {
          $(this).addClass('mentalSwatch');
        }
        for (var i = 0; i < dataChatter.length; i++) {
          if (dataChatter[i].category == 'location_all') {
            $('.chatterThing').html(dataChatter[i].chatter);
          }
        }
      }
  ...
  $('#mannerB').on('click', function() {
    ...
    $('.dot').each(function(i, obj) {
      if (mentalLoad == true) {
        if (
          $(this).attr('manner') == 'HOMICIDE' &&
          $(this).attr('mental') == 'Yes'
        ) {
          $(this).addClass('mentalSwatch');
          for (var i = 0; i < dataChatter.length; i++) {
            if (dataChatter[i].category == 'manner_mental') {
              $('.chatterThing').html(dataChatter[i].chatter);
            }
          }
        }
        ...
        if ($(this).attr('manner') == 'HOMICIDE') {
          $(this).addClass('mentalSwatch');
        }
        for (var i = 0; i < dataChatter.length; i++) {
          if (dataChatter[i].category == 'manner_all') {
            $('.chatterThing').html(dataChatter[i].chatter);
          }
        }
      }
  ...
  $('#genderB').on('click', function() {
    ...
    $('.dot').each(function(i, obj) {
      if (mentalLoad == true) {
        if (
          $(this).attr('gender') == ' MALE' &&
          $(this).attr('mental') == 'Yes'
        ) {
          $(this).addClass('mentalSwatch');
          for (var i = 0; i < dataChatter.length; i++) {
            if (dataChatter[i].category == 'gender_mental') {
              $('.chatterThing').html(dataChatter[i].chatter);
            }
          }
        }
        ...
        for (var i = 0; i < dataChatter.length; i++) {
          if (dataChatter[i].category == 'gender_all') {
            $('.chatterThing').html(dataChatter[i].chatter);
          }
        }
      }
